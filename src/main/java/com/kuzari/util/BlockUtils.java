...
public class BlockUtils {
    ...
    public static boolean isNearFence(final Player p) {
        boolean out = false;
        for (final Block b : getNearbyBlocks(p.getLocation(), 3)) {
            if (isFence(b)) {
                out = true;
            }
        }
    ...
    public static boolean isNearWall(final Player p) {
        boolean out = false;
	...
	public static boolean isWeb(Block block) {
		if(block.getType() == Material.WEB || block.getType() == Material.WEB) {
			return true;
		}
		return false;
	}
     ...
     */
    static String[] HalfBlocksArray = { "pot",  "step", "slab", "snow", "detector", "daylight",
            "comparator", "repeater", "water", "lava", "ladder", "vine", "carpet", "sign", "pressure", "plate",
            "button", "mushroom", "torch", "frame", "armor", "banner", "lever", "hook", "redstone", "rail", "brewing",
             "skull", "enchantment", "cake","bed" };
    ...
    public static boolean isNearHalfBlock(Player p) {
        boolean out = false;
         for (Block b : getNearbyBlocks(p.getLocation(), 3)) {
             if (isHalfBlock(b)) {
                 out = true;
             }
         }
